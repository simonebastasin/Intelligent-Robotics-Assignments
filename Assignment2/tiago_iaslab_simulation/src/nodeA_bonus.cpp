...
const vector<float> pickGreenPose {6.579991 + 1.186584 - 0.2, - 1.369981 - 1.967163 - 0.7, 90 - 10}; 
const vector<float> pickBluePose {6.579991 + 1.521553 - 0.2, - 1.369981 - 1.278487 + 0.7, - 90}; 
const vector<float> pickRedPose {6.579991 + 0.939518 + 0.2, - 1.369981 - 1.284885 + 0.7, - 90}; 

const vector<float> placeRedPose {6.579991 + 4.007962 - 0.1, - 1.369981 + 1.015966 + 0.7, - 90};
const vector<float> placeGreenPose {6.579991 + 5.007404 - 0.1, - 1.369981 + 1.015966 + 0.7, - 90};
const vector<float> placeBluePose {6.579991 + 6.007146 - 0.1, - 1.369981 + 1.015966 + 0.7, - 90};
...
const vector<float> detectPlacePose {6.579991 + 5, - 1.369981 + 3, - 90};
...
void moveToPose(int id, bool going_to_place) {
	...
	else { // pose to detect place tables
		goal_pose.x = detectPlacePose[0];
		goal_pose.y = detectPlacePose[1];
		goal_pose.yaw_degrees = detectPlacePose[2];
	}	
	ac.sendGoal(goal_pose, &doneCb, &activeCb, &feedbackCb);
	ac.waitForResult();
}
...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < ids.size(); i++) {
		...
		moveHead(true);
		moveTorso(true);
		moveToPose(ids[i], true); // move to place tables detection

		// ---------- OBSTACLES DETECTION ----------

		ROS_INFO("getting current pose from topic (/robot_pose)");

		ros::NodeHandle pose_n;
		boost::shared_ptr<geometry_msgs::PoseWithCovarianceStamped const> sharedPose;
		geometry_msgs::Pose msg_pose;
		sharedPose = ros::topic::waitForMessage<geometry_msgs::PoseWithCovarianceStamped>("/robot_pose",pose_n);
		if(sharedPose != NULL) msg_pose = sharedPose->pose.pose;
		tf2::Quaternion get_pose;
		double get_roll,get_pitch,get_yaw;
		tf2::fromMsg(msg_pose.orientation, get_pose);
        tf2::Matrix3x3(get_pose).getRPY(get_roll, get_pitch, get_yaw);

		ROS_INFO("topic pose: x = %f, y = %f, yaw = %f", msg_pose.position.x,msg_pose.position.y,get_yaw * M_PI / 180.0);

		// Call laserScan node through a service to get obstacle centers
		...
		//srv.request.x = msg_pose.position.x;
        //srv.request.y = msg_pose.position.y;
        srv.request.yaw_radians = get_yaw * M_PI / 180.0;
		
        srv.request.x = detectPlacePose[0];
        srv.request.y = detectPlacePose[1];
        //srv.request.yaw_radians = detectPlacePose[2] * M_PI / 180.0;
		
		ros::Duration(3.0).sleep();
		
		vector<float> x_positions, x_positions_wrf, y_positions, y_positions_wrf;
		// invoke an RPC service with [service: contains req and res msgs], returns true on success
		if (client.call(srv)) {
			ROS_INFO("Detection done: sending back response to client");
			// copy obstacle positions in action result
			for (int j = 0; j < srv.response.x_positions.size(); j++) {
				// robot reference frame
				//ROS_INFO("x = %f", srv.response.x_positions[j]);
				//ROS_INFO("y = %f", srv.response.y_positions[j]);
				//ROS_INFO("a = %f", srv.response.angle_positions[j]);
				// world reference frame
				ROS_INFO("x = %f", srv.response.x_positions_wrf[j]);
				ROS_INFO("y = %f", srv.response.y_positions_wrf[j]);
			}

			actionlib::SimpleActionClient<tiago_iaslab_simulation::ClientProxyAction> ac1("/proxy_topic", true);
			ROS_INFO("Waiting for action proxy to start");
			ac1.waitForServer();
			// goal to action proxy to move robot
			tiago_iaslab_simulation::ClientProxyGoal goal_pose;

			goal_pose.x = srv.response.x_positions_wrf[ids[i]-1];
			goal_pose.y = srv.response.y_positions_wrf[ids[i]-1] + 0.7;
			goal_pose.yaw_degrees = -90;	
			ROS_INFO("place pose: x = %f, y = %f, yaw = %f", goal_pose.x,goal_pose.y,goal_pose.yaw_degrees);
			ac1.sendGoal(goal_pose, &doneCb, &activeCb, &feedbackCb);
			ac1.waitForResult();	
		}
		else {
		...
		/*
		actionlib::SimpleActionClient<tiago_iaslab_simulation::ClientProxyAction> ac("/proxy_topic", true);
		ROS_INFO("Waiting proxy");
		ac.waitForServer();
		tiago_iaslab_simulation::ClientProxyGoal goal_pose;
        goal_pose.x = detectPlacePose[0];
        goal_pose.y = detectPlacePose[1];
        if (ids[i] == 1) {
            goal_pose.x += srv.response.y_positions[2];
            goal_pose.y -= srv.response.x_positions[2];
            goal_pose.yaw_degrees = srv.response.angle_positions[2];
        }	
        else if (ids[i] == 2) {
            goal_pose.x += srv.response.y_positions[1];
            goal_pose.y -= srv.response.x_positions[1];
            goal_pose.yaw_degrees = srv.response.angle_positions[1];
        }	
        else if (ids[i] == 3) {
            goal_pose.x += srv.response.y_positions[0];
            goal_pose.y -= srv.response.x_positions[0];
            goal_pose.yaw_degrees = - srv.response.angle_positions[0];
        }	
		ac.sendGoal(goal_pose, &doneCb, &activeCb, &feedbackCb);
		ac.waitForResult();
		ros::Duration(3.0).sleep();
